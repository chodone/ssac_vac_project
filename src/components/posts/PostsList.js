...
function PostItem({ post, onClickPost }) {
    ...
    const inoInfoObject = writer.inoInfo[Number(writer.inoInfo.length)-1] 
    const calAge = codemgmt.getAge(writer.bDay)
    console.log(calAge)
    console.log(writer.nickName)
      ...
      <PostsListContainer>
        {posts &&
          posts.map((post, index) => {
            return <PostItem post={post} key={index} onClickPost={() => {
